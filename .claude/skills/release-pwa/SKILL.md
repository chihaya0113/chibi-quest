...
3. `npm run serve` で起動し、以下を確認する:
   - **確認の前に検証用ブラウザの古いキャッシュを必ず捨てる**。旧Service Workerが旧ファイルを返し続け、「反映されている」の確認が誤判定になるため。コンソールで `navigator.serviceWorker.getRegistrations()` から全unregister + `caches.keys()` から全deleteしてリロードする
   - 変更内容が画面に反映されている
   - ブラウザのコンソールに Service Worker のエラーが出ていない
   - 既存セーブがある状態で正常に起動する(save-data-compat 参照)
   - 補足: 画像(src/assets)と問題データ(questions.js)には `?v=` が付かない(ランタイムキャッシュ)。利用者への反映は CACHE_VERSION 更新が唯一の手段で、検証時にこれらの変更が見えないときは `?cb=1` 等のクエリを一時的に付けて確認する
